...
const AdminLogin = ({ onLogin, onBack }) => {
  ...
  const isValidAdminId = (value) => /^ADM-[0-9]{4,}$/.test(value.trim().toUpperCase());
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    setSuccess('');
    if (!isValidAdminId(uniqueId)) {
      setError('Enter a valid Admin ID in ADM-1234 format.');
      return;
    }
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/signup/', {
        ...
        body: JSON.stringify({ full_name: fullName.trim(), unique_id: normalizedId, password, role: 'admin' }),
      });
      ...
      setSuccess('Administrator account created successfully. You can now log in.');
      setMode('login');
    } catch (err) {
      ...
      >
        ...
        <div className="login-buttons">
          <button type="submit" className="btn-login">
            {mode === 'login' ? 'Log In' : mode === 'signup' ? 'Create Admin Account' : (otpSent ? 'Verify OTP and Reset Password' : 'Send OTP')}
          </button>
